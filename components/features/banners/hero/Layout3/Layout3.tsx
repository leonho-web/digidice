...
const FeaturedGameCard = ({ game }: { game: Game }) => {
	...
	return (
		<Card className="group relative overflow-hidden border-0 shadow-2xl hover:shadow-3xl transition-all duration-500 h-[400px] xl:h-[420px] 2xl:h-[460px] cursor-pointer">
...
type CompactGameCardProps = { game: Game; heightClass?: string };
const CompactGameCard = ({
	...
	return (
		<Card
			className={`group relative overflow-hidden border-0 shadow-lg hover:shadow-xl transition-all duration-300 cursor-pointer ${heightClass}`}
		>
			{/* Full Background Image */}
			<div className="absolute inset-0">
				<Image
...
export const Layout3 = ({
	...
	const exploreBottomPlugin = useRef(
		Autoplay({
			delay: 2500,
			stopOnInteraction: false,
			stopOnMouseEnter: true,
	...
	// Split side games for two carousels
	const midPoint = Math.ceil(sideGames.length / 2);
	const topExploreGames = sideGames.slice(0, midPoint);
	const bottomExploreGames = sideGames.slice(midPoint);
	return (
		<div className="w-full space-y-8 py-8">
			...
			<div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
				...
				{/* Slider 2: More to Explore - Split into two carousels */}
				<div className="space-y-6">
					<div className="flex items-center justify-between">
						<h2 className="text-2xl font-bold text-foreground">
					...
					<div className="relative">
						...
						>
							<CarouselContent>
								{topExploreGames.map((game, index) => (
									<CarouselItem
										key={`explore-top-${game.game_id}-${index}`}
										className="basis-full sm:basis-1/2"
									>
										<CompactGameCard
											game={game}
											heightClass="h-[300px] xl:h-[190px] 2xl:h-[220px]"
										/>
									</CarouselItem>
								))}
							</CarouselContent>
					...
					{/* Bottom Carousel - Different direction */}
					<div className="relative">
						<Carousel
							plugins={[exploreBottomPlugin.current]}
							className="w-full"
							opts={{
								align: "start",
								loop: true,
						...
						>
							<CarouselContent className="flex-row-reverse">
								{bottomExploreGames.map((game, index) => (
									<CarouselItem
										key={`explore-bottom-${game.game_id}-${index}`}
										className="basis-full sm:basis-1/2"
									>
										<CompactGameCard
											game={game}
